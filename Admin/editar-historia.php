<?php
    ...
    else require_once 'Views/editar-historia.view.php';

    function Cargar(){
        $conexion = conectar();
        $id = $_GET['id'];

            $buscarHistoria = $conexion->prepare("SELECT * FROM historia WHERE ID = ? ");
            $buscarHistoria->execute([$id]);
            $Historia = $buscarHistoria->fetchAll();

            foreach($Historia as list($idHistorial, $titulo, $subtitulo, $contenido, $imagen)){
                ...
                echo"
                    ...
                    <div class='form-group'> <label class='form-control-label'>Fecha:</label> <input onkeypress='javascript:return notnumprotection(event)'' type='text' id='subtitulo' class='form-control' onblur='validate1(5)' value='$subtitulo'></div>

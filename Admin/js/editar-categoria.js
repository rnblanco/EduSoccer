...
function validate1(val) {
    ...
    v6 = document.getElementById("profesor");
...
$(document).ready(function(){
        ...
        $(".next").click(function(e){
            ...
            //con imagen
            if( validate1(1) && validate1(2) && validate1(3) && validate1(4) && validate1(5) && validate1(6)){
                ...
                if(jQuery.inArray(ext, ['png','jpg','jpeg']) == -1){
                    ...
                    if(fsize > 2000000){
                        ...
                        form_data.append("subtitulo", subtitulo);form_data.append("id",id);form_data.append("profesor",profesor);
                        $.ajax({
                            url:'../Db/editarCategoriaConImagen.php',
                            data:form_data,
                            cache: false,
                            contentType: false,
                            processData: false,
                            type:'post',
                            success:function(){
                                Swal.fire('Categoría editada correctamente! ','','success').then((result)=>{
                                    if(result.value){
                                ...
                                Swal.fire('La Categoria no fue editada con imagen','','error');
                            }
                        });
                    }
                }
            ...
            //sin imagen
            else if( validate1(1) && validate1(2) && validate1(3)==false && validate1(4) && validate1(5) && validate1(6)){

                var form_data = new FormData();
                        form_data.append("contenido", contenido);form_data.append("titulo", titulo);
                        form_data.append("subtitulo", subtitulo);form_data.append("id",id);form_data.append("profesor",profesor);
                        $.ajax({
                            url:'../Db/editarCategoriaSinImagen.php',
                            data:form_data,
                            cache: false,
                            contentType: false,
                            processData: false,
                            type:'post',
                            success:function(){
                                Swal.fire('Categoria editada correctamente! ','','success').then((result)=>{
                                    if(result.value){
                                ...
                                Swal.fire('No has hecho cambios en la Categoria','','error');
                            }
                        });   
            }
            else{

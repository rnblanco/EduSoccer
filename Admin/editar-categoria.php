<?php
        ...
        foreach($Categoria as list($idCategorial, $titulo, $subtitulo, $contenido, $profesor, $imagen)){
            ...
            echo"
                ...
                <div class='form-group'> <label class='form-control-label'>Profesor:</label>
                    <select type='text' id='profesor' class='form-control' onblur='validate1(6)'>";Usuarios($profesor);
					echo"</select>
				</div>
	...
	function Usuarios($id){
		...
		foreach($Usuarios as list($id2, $usuario, $pass, $nombre, $apellido, $edad, $cargo, $estado)){
			$nombreCompleto = $nombre . " " . $apellido;
			if($id==$id2) echo" <option value='$id2' selected='selected'> $nombreCompleto </option> ";
			else echo" <option value='$id2'> $nombreCompleto </option> ";
		}
	}
?>